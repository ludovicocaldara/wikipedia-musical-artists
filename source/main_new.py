...
if len(args) > 0:
    ...
    iterations = 10 # pylint: disable=C0103
    while iterations > 0:
        from MongoFactory import mongo_db
        coll = mongo_db['artist_crawling']
        ...
        for new_band in bands_to_discover:
            # if new_band link is there, use the link, otherwise try with name
            if new_band.get('link') is not None:
                process_band(new_band['link'])
            else:
                process_band(new_band['name'])
        iterations -= 1


# TODO: figure out how to manage omonyms... Scott Reeder exists as (bassist) and (drummer) :-(
